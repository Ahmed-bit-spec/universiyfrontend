...
import { Eye, MoreHorizontal, Pencil, Trash2, Link2, Link2Off } from "lucide-react";
import { cn } from "@/lib/utils";

const UniversityStudentActionDropdown = ({ labels, student, onView, onEdit, onDelete, onToggleClaimed }) => {
            ...
            <AnimatePresence>
                {open && (
                    ...
                    >
                        ...
                        <button type="button" className={itemClass} onClick={() => run(onToggleClaimed)}>
                            {student.isClaimed ? (
                                <>
                                    <Link2Off size={14} /> {labels.markUnclaimed}
                                </>
                            ) : (
                                <>
                                    <Link2 size={14} /> {labels.markClaimed}
                                </>
                            )}
                        </button>
                        <button
...
export default UniversityStudentActionDropdown;
