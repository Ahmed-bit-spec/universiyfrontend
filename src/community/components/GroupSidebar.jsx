...
import { Users, GraduationCap, Layers, BookOpen, Search, Building2 } from "lucide-react";
...
// Colors untouched — same Duolingo green tokens as before. Only structure/typography changed.
const BTN_PRIMARY =
...
const EMPTY_GROUPS = { university: [], department: [], teacherGroups: [], courses: [], classes: [] };

/**
 * Sidebar section ORDER is role-driven per spec:
 *   Student:  UNISO Community -> Department -> Class -> Course        (no Teacher Group)
 *   Teacher:  UNISO Community -> Teacher Group -> Department -> Class -> Course
 *   Admin:    UNISO Community -> Department -> Teacher Group -> Course -> Class
 * Students are never handed a "teacherGroups" section at all — they simply have no
 * membership there, so there is nothing to filter out defensively; the backend guarantees it.
 */
const sectionsForRole = (role) => {
  const base = {
    university: { key: "university", label: "UNISO Community", icon: Building2 },
    department: { key: "department", label: "Department Group", icon: Users },
    teacherGroups: { key: "teacherGroups", label: "Teacher Group", icon: GraduationCap },
    classes: { key: "classes", label: "Class Groups", icon: Layers },
    courses: { key: "courses", label: "Course Groups", icon: BookOpen },
  };

  if (role === "teacher") return [base.university, base.teacherGroups, base.department, base.classes, base.courses];
  if (role === "admin") return [base.university, base.department, base.teacherGroups, base.courses, base.classes];
  return [base.university, base.department, base.classes, base.courses]; // student
};
...
const GroupCard = ({ group }) => (
  ...
  >
    <div className="flex items-start gap-4">
      ...
      <span className={`px-4 py-2 rounded-xl ${BTN_PRIMARY} shrink-0`}>Open</span>
    </div>
  </Link>
);
...
const Groups = () => {
  const { t } = useLanguage();
  const { user } = useAuth();
  const SECTIONS = useMemo(() => sectionsForRole(user?.role), [user?.role]);

  const [groups, setGroups] = useState(EMPTY_GROUPS);
  const [loading, setLoading] = useState(true);
  const [active, setActive] = useState(SECTIONS[0]?.key ?? "university");
